<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Menu</title>
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.1.3/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-1BmE4kWBq78iYhFldvKuhfTAU6auU8tT94WrHftjDbrCEXSU1oBoqyl2QvZ6jIW3" crossorigin="anonymous">
</head>
<body class=" bg-opacity-25">
    <div class="container">
        <div class="">
            <p class="text-center display-2 border-bottom border-dark">MENU</p>
        </div>
        <div class="card-group">
            <div class="card m-2 border-1 shadow-lg">
                    <img class="card-img-top" src="https://w0.peakpx.com/wallpaper/129/948/HD-wallpaper-delicious-food-meal-dish-delicious-food-fries-meat-vegetables-thumbnail.jpg" alt="Card image cap"/>
                    <div class="card-body">
                        <h4 class="card-title">Nombre del Platillo</h4>
                        <p class="card-text">Precio: Q. </p>
                        <span>
                            Lorem ipsum dolor sit amet, consectetur adipisicing elit. Eligendi dolorem eum dicta, deleniti ab aut numquam voluptatum laudantium eos nesciunt pariatur quidem voluptatem reiciendis soluta at enim, sit ullam. Maxime!
                        </span>
                        <div class="d-flex pt-3">
                            <button class="btn btn-sm btn-warning text-white m-auto" type="button" data-bs-toggle="collapse" data-bs-target="#contentId" aria-expanded="false"
                                    aria-controls="contentId">
                                ORDENAR
                            </button>
                        </div>
                </div>
            </div>
            <div class="card border-2 m-2 border-1 shadow-lg">
                <img class="card-img-top" src="https://c1.wallpaperflare.com/preview/670/756/865/hotdog-delicious-yummy-fresh-fries.jpg" alt="Card image cap">
                <div class="card-body">
                    <h4 class="card-title">Nombre del Platillo</h4>
                    <p class="card-text">Precio: Q. </p>
                    <span>
                        Lorem ipsum dolor sit amet, consectetur adipisicing elit. Eligendi dolorem eum dicta, deleniti ab aut numquam voluptatum laudantium eos nesciunt pariatur quidem voluptatem reiciendis soluta at enim, sit ullam. Maxime!
                    </span>
                    <div class="d-flex pt-3">
                        <button class="btn btn-sm btn-warning m-auto text-white" type="button" data-bs-toggle="collapse" data-bs-target="#contentId" aria-expanded="false"
                                aria-controls="contentId">
                            ORDENAR
                        </button>
                    </div>
                </div>
            </div>
        </div>
    </div>
</body>
</html>